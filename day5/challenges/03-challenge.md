...
For each environmnent two applications are registerd. One for the client and one for all APIs of the sample application.

### Run the script

Open a shell and use Azure CLI to connect to the Azure AD Tenant where you want to create the applications (you can also use the Azure Cloud Shell). If you have created a new Azure AD that is not linked to an Azure subscription, add the additional option _--allow-no-subscription_:
...
a _Dev_ and _Test_ stage for Day5 with Azure AD integration. A GitHub environment can be configured with
...
::: warning
⚠️ Make sure you use the values you used for the application registration script for the development environment !
:::
...
::: warning
⚠️ Make sure you use the values you used for the application registration script for the test environment !
:::
...
Service principal in our Azure Active Directory. If you have already created a service principal and added it to your repository's secrets, you can skip this step and continue with [Activate the CI/CD workflow](#activate-the-cicd-workflow).
...
Now we need to store the Service Principal's credentials. Navigate to the imported
...
Now we have everything prepared to active the CI/CD workflow. First, checkout the master branch, pull changes and 
create a new branch `cicd/aad-common`
...
the repository and created a new branch `cicd/aad-common`.
...
Now, create a pull request to merge the branch `cicd/aad-common` into the `master`
branch. Set `Deploy shared Azure resources for AAD integration` as title.
...
In VS Code, open the workflow `day5-scm-frontend.yml` and replace the organization name in
each job condition with your organization's name:
...
Now merge the pull request and navigate to your repository' action page. You
should see that the `day5-scm-frontend` workflow is triggered after a few seconds.

The workflow is now in the `waiting`state, because a reviewer must approve the deployment to the `test`
environment:
